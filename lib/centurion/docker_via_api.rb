...
class Centurion::DockerViaApi
  def initialize(hostname, port, tls_args = {}, api_version = nil)
    ...
    @api_version = api_version.nil? ? '1.7' : api_version

    configure_excon_globally
  end

  def ps(options={})
    path = "/v#{@api_version}/containers/json"
  ...
  def inspect_image(image, tag = "latest")
    ...
    path       = "/v#{@api_version}/images/#{repository}/json"
  ...
  def remove_container(container_id)
    path = "/v#{@api_version}/containers/#{container_id}"
  ...
  def stop_container(container_id, timeout = 30)
    path = "/v#{@api_version}/containers/#{container_id}/stop?t=#{timeout}"
    response = Excon.post(
      @base_uri + path,
      tls_excon_arguments
    )
  ...
  def create_container(configuration, name = nil)
    path = "/v#{@api_version}/containers/create"
    response = Excon.post(
      @base_uri + path,
  ...
  def start_container(container_id, configuration)
    path = "/v#{@api_version}/containers/#{container_id}/start"
    response = Excon.post(
      @base_uri + path,
      tls_excon_arguments.merge(
        body: configuration.to_json,
  ...
  def restart_container(container_id, timeout = 30)
    path = "/v#{@api_version}/containers/#{container_id}/restart?t=#{timeout}"
    response = Excon.post(
      @base_uri + path,
      tls_excon_arguments
    )
  ...
  def inspect_container(container_id)
    path = "/v#{@api_version}/containers/#{container_id}/json"
    response = Excon.get(
      @base_uri + path,
      tls_excon_arguments
    )
